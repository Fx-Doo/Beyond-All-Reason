...
function gadget:AllowCommand(unitID, unitDefID, teamID, cmdID, cmdParams, cmdOptions, cmdTag, playerID, fromSynced, fromLua)
	...
	and UnitDefs[GG.GetUnitDefID(cmdParams[1])] ~= nil then
		if unEmpableUnits[GG.GetUnitDefID(cmdParams[1])] then		--	and UnitDefs[GG.GetUnitDefID(cmdParams[1])].customParams.paralyzemultiplier == '0' then
			return false
		else
			local _,_,_,_,y = GG.GetUnitPosition(cmdParams[1], true)
			if y and y >= 0 then
				return true
			else
				return false
