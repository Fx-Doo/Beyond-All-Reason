...
if (not gadgetHandler:IsSyncedCode()) then
	...
	function gadget:AllowCommand(unitID, unitDefID, unitTeam, cmdID, cmdParams, cmdOptions, cmdTag, synced)
		...
		if idleStates[unitID] == 1 and not cmdOptions.internal then -- auto generated targets from fight cmds wont trigger allowcommand so we're not removing ourselves from idle state prematurely
			-- although i'd have to figure out what happens with non queueing (ie state commands) or custom commands that should be stopped by allowcommand and processed from game side (other gadgets)
			-- what i think is non queuing cmds should instantly trigger unitcmdDone and unitidle when performed
			-- and if layer makes this called last then all custom commands that aren't queued should have been processed and filtered out by now
			RemoveIdleTag(unitID)
			local cmdQueue = Spring.GetUnitCommands(unitID, -1)
			for i = 1,#cmdQueue do
				Spring.GiveOrderToUnit(unitID, CMD.REMOVE, {cmdQueue[i].tag}, {})
			end
			return true
		end
		return true
