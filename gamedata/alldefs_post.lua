...
local targetingProximityPriorities = VFS.Include("gamedata/targeting_proximity_priorities.lua")
local aimAdjustPriorities          = VFS.Include("gamedata/aim_adjust_priorities.lua")
...
local function unitDef_Post(name, uDef)
	...
	if next(weapondefs) then
		...
		for weaponName, weaponDef in pairs(weapondefs) do
			local techLevel = tonumber(uDef.customparams.techlevel or 1)
			local dumbness = math.min(1,math.max(0, 1 - (techLevel / 3)))
			weaponDef.customparams.dumbness = dumbness
			local groupNumber = 0
	...
	-- weaponAimAdjustPriority from config file (generated by gadget).
	-- Keys are unitDef name strings; looked up via UnitDefNames.
	if aimAdjustPriorities then
		if aimAdjustPriorities[name] then
			local wpns = aimAdjustPriorities[name]
			local wSlots = uDef.weapons or {}
			for wNum, p in pairs(wpns) do
				if wSlots[wNum] then
					wSlots[wNum].weaponAimAdjustPriority = 0
				end
			end
		end
	end
...
local function weaponDef_Post(name, wDef)
	...
	if not SaveDefsToCustomParams then
		...
		-- Apply empirically calibrated proximityPriority (from targeting_proximity_priorities.lua).
		-- Overrides any value set in the weapon def; the clamp below still applies.
		local range = wDef.range or 1
		wDef.proximitypriority = 0.4 + (100 / range)
